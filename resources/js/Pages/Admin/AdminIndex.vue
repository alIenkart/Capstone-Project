<template>
  <div :class="[
    ...
    <div :class="[
      ...
      <svg class="animate-spin h-12 w-12 text-[#1E71B8]" xmlns="http://www.w3.org/2000/svg" fill="none"
        viewBox="0 0 24 24">
    ...
    <aside :class="[
      ...
      <div class="flex flex-col">
        <div class="flex items-center justify-between px-6 py-8 border-b border-white/10" v-if="isSidebarOpen">
          <div class="flex items-center gap-3">
            ...
            <span class="text-white text-2xl font-bold tracking-tight">{{adminName.toUpperCase()}}</span>
          </div>
          <button @click="toggleSidebar" class="text-white/80 hover:text-white focus:outline-none">
        ...
        <div v-else class="flex items-center justify-center px-3 py-8 border-b border-white/10 cursor-pointer"
          @click="toggleSidebar">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
    ...
    <div v-if="isSidebarOpen && !isLoggingOut" class="fixed inset-0 bg-black/40 z-40 lg:hidden" @click="toggleSidebar">
    </div>
    ...
    <div :class="[
      ...
      <header
        ...
        <div class="mb-1">
          <h1 class="text-3xl font-bold text-gray-800 mb-2">{{ headerTitle.title }}</h1>
...
const headerTitle = computed(() => {
  if (page.url.startsWith('/admin/admindashboard'))
    return { title: 'Dashboard', subtitle: 'Overview of system statistics and activity' }
  if (page.url.startsWith('/admin/users'))
    return { title: 'User Management', subtitle: 'Manage system users and their permissions' }
  if (page.url.startsWith('/admin/packages'))
    return { title: 'Package Management', subtitle: 'Create and manage tour packages' }
  if (page.url.startsWith('/admin/booking-entries'))
    return { title: 'Booking Entries', subtitle: 'Manage and track all customer bookings' }
  if (page.url.startsWith('/admin/payment-confirmation'))
    return { title: 'Payment Management', subtitle: 'Review and approve customer payment submissions' }
