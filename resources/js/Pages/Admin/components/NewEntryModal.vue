<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fadeIn">
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-6xl mx-4 max-h-[95vh] overflow-hidden flex flex-col animate-slideUp">
      <button
      ...
      <div class="px-8 pt-8 pb-6">
        <h2 class="text-center text-2xl font-bold bg-gradient-to-r from-[#1E71B8] to-[#73BE5D] bg-clip-text text-transparent mb-8">
        ...
        <div class="flex items-center gap-3 mb-8 justify-center">
          <div v-for="(title, index) in stepTitles" :key="index" class="flex items-center gap-3">
            <div class="relative">
              ...
              <div v-if="currentStep >= index" class="absolute inset-0 rounded-full bg-gradient-to-br from-[#1E71B8] to-[#73BE5D] animate-ping opacity-20"></div>
            </div>
            <div v-if="index < stepTitles.length - 1" :class="[
              'w-16 h-1 rounded-full transition-all duration-500',
              currentStep > index ? 'bg-gradient-to-r from-[#1E71B8] to-[#73BE5D]' : 'bg-gray-200'
            ]"></div>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto px-8 pb-8">
        <div v-if="currentStep === 0" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div class="space-y-6">
            <div class="group">
              <label class="block font-semibold mb-3 text-[#1E71B8] text-sm uppercase tracking-wide">Customer Name</label>
              <input
            ...
            <div class="group">
              <label class="block font-semibold mb-3 text-[#1E71B8] text-sm uppercase tracking-wide">Package Name</label>
              <div class="relative" ref="packageDropdownRef">
                <button type="button"
                  ...
                  :class="[
                    ...
                    selectedPackage === '' ? 'border-gray-200 text-gray-400' : 'border-[#1E71B8] text-gray-900'
                  ]"
                >
                  <span class="font-semibold">{{ selectedPackage ? packages.find(p => p.id === selectedPackage)?.package_name : 'Select Package' }}</span>
                  <svg :class="['w-5 h-5 transition-transform duration-300', isPackageOpen ? 'rotate-180' : '']"
                ...
                <div v-if="isPackageOpen" @click.stop
                  ...
                  <div v-for="pkg in packages" :key="pkg.id" @click="handlePackageSelect(pkg.id)"
                    :class="[
                    ...
                    <span :class="['font-medium', selectedPackage === pkg.id ? 'text-[#1E71B8]' : 'text-gray-700']">
          ...
          <div class="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white rounded-3xl p-8 min-h-[300px] shadow-xl relative overflow-hidden">
            ...
            <div class="relative z-10">
              ...
              <div v-if="selectedPackageData.id" class="space-y-4">
                <div>
                  ...
                  <div class="text-xl font-bold text-[#73BE5D]">{{ selectedPackageData.package_name }}</div>
                </div>
                <div>
                  <div class="text-xs text-gray-400 mb-1">Destination</div>
                  <div class="font-semibold">{{ selectedPackageData.destination }}</div>
                </div>
                <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-700">
                  <div>
                    <div class="text-xs text-gray-400 mb-1">Adult Rate</div>
                    <div class="text-lg font-bold text-white">₱{{ selectedPackageData.pax_rate }}</div>
                  </div>
                  <div>
                    <div class="text-xs text-gray-400 mb-1">Kids Rate</div>
                    <div class="text-lg font-bold text-white">₱{{ selectedPackageData.kids_pax_rate }}</div>
                  </div>
                </div>
              </div>
          ...
          <div class="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="relative z-50" ref="typeDropdownRef">
              ...
              <button type="button"
                ...
                <svg :class="['w-5 h-5 transition-transform duration-300', isTypeOpen ? 'rotate-180' : '']" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path>
                </svg>
              </button>
              <div v-if="isTypeOpen" class="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-gray-100 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-slideDown max-h-60 overflow-y-auto">
                <a v-for="t in tourTypes" :key="t" href="#" class="block px-5 py-3 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200"
                  :class="{ 'bg-gradient-to-r from-blue-50 to-indigo-50 text-[#73BE5D] font-semibold border-l-4 border-[#73BE5D]': t === tourType }" @click.prevent="selectTourType(t)">
                  <div class="flex items-center justify-between">
                    <span>{{ t }}</span>
                    <svg v-if="t === tourType" class="w-5 h-5 text-[#73BE5D] animate-checkmark" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="relative z-50" ref="classDropdownRef">
              <label class="block text-[#1E71B8] mb-3 font-semibold text-sm uppercase tracking-wide">Tour Classification</label>
              <button type="button"
                class="w-full border-2 border-gray-200 rounded-2xl px-5 py-4 bg-white text-left text-[#1E71B8] flex items-center justify-between hover:border-[#1E71B8] hover:shadow-md transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-[#1E71B8]/10"
                @click="isClassOpen = !isClassOpen; isTypeOpen = false">
                <span class="font-semibold">{{ tourClassification }}</span>
                <svg :class="['w-5 h-5 transition-transform duration-300', isClassOpen ? 'rotate-180' : '']" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path>
                </svg>
              </button>
              <div v-if="isClassOpen" class="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-gray-100 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-slideDown max-h-60 overflow-y-auto">
                <a v-for="c in tourClassifications" :key="c" href="#" class="block px-5 py-3 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200"
                  :class="{ 'bg-gradient-to-r from-blue-50 to-indigo-50 text-[#73BE5D] font-semibold border-l-4 border-[#73BE5D]': c === tourClassification }" @click.prevent="selectTourClassification(c)">
                  <div class="flex items-center justify-between">
                    <span>{{ c }}</span>
                    <svg v-if="c === tourClassification" class="w-5 h-5 text-[#73BE5D] animate-checkmark" fill="currentColor" viewBox="0 0 20 20">
                      <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                    </svg>
                  </div>
                </a>
          ...
          <div class="flex flex-col lg:flex-row gap-8">
            <div class="lg:w-48 flex-shrink-0">
              <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 shadow-sm">
                <h3 class="text-sm font-bold text-[#1E71B8] mb-4 uppercase tracking-wide">Legends</h3>
                <div class="space-y-3">
                  <div class="flex items-center gap-3 group">
                    <span class="w-4 h-4 rounded-full bg-[#d9d9d9] shadow-sm group-hover:scale-110 transition-transform"></span>
                    <span class="text-sm text-gray-700 font-medium">Closed</span>
                  </div>
                  <div class="flex items-center gap-3 group">
                    <span class="w-4 h-4 rounded-full bg-gray-800 shadow-sm group-hover:scale-110 transition-transform"></span>
                    <span class="text-sm text-gray-700 font-medium">Available</span>
                  </div>
                  <div class="flex items-center gap-3 group">
                    <span class="w-4 h-4 rounded-full bg-[#2f5f98] shadow-sm group-hover:scale-110 transition-transform"></span>
                    <span class="text-sm text-gray-700 font-medium">Slots Full</span>
                  </div>
                  <div class="flex items-center gap-3 group">
                    <span class="w-4 h-4 rounded-full bg-[#45a834] shadow-sm group-hover:scale-110 transition-transform"></span>
            ...
            <div class="flex-1">
              <div class="bg-white rounded-3xl p-8 shadow-xl border-2 border-gray-100 relative z-0">
                <div v-if="tourType === 'Exclusive'" class="mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-[#1E71B8] p-4 rounded-xl">
                  <div class="flex items-start gap-3">
                    ...
                    <div>
                      <p class="text-sm font-semibold text-[#1E71B8] mb-1">Select date range</p>
                      <p class="text-xs text-gray-600">Click start date, then click end date to complete your selection.</p>
                    </div>
                  </div>
                </div>

                <div class="flex items-center justify-between mb-6">
                  <button @click="prevMonth" class="w-10 h-10 rounded-xl border-2 text-[#1E71B8] border-[#1E71B8] hover:bg-[#1E71B8] hover:text-white transition-all duration-300 font-bold flex items-center justify-center hover:scale-110">‹</button>
                  <div class="text-xl font-bold text-[#1E71B8] uppercase tracking-wide">{{ monthName }} {{ currentYear }}</div>
                  <button @click="nextMonth" class="w-10 h-10 rounded-xl border-2 text-[#1E71B8] border-[#1E71B8] hover:bg-[#1E71B8] hover:text-white transition-all duration-300 font-bold flex items-center justify-center hover:scale-110">›</button>
                </div>
                ...
                <div class="grid grid-cols-7 gap-2 relative" ref="calendarWrapperRef">
                  <div v-for="(cell, i) in calendarDays" :key="i"
                    ...
                    :class="{
                      'bg-gradient-to-br from-[#73BE5D] to-[#5fa84d] text-white shadow-lg scale-105': cell.isInRange && !cell.isPast,
                      'bg-gray-100 text-gray-400 cursor-not-allowed opacity-40': !cell.day || cell.isPast,
                      'bg-[#d9d9d9] text-gray-700 cursor-pointer hover:scale-110 hover:shadow-md': cell.day && cell.status === 'closed' && !cell.isPast && !cell.isInRange,
                      'bg-gradient-to-br from-[#45a834] to-[#3a8f2b] text-white cursor-pointer hover:scale-110 hover:shadow-lg': cell.day && cell.status === 'accepting' && !cell.isPast && !cell.isInRange,
                      'bg-gradient-to-br from-[#2f5f98] to-[#254d7d] text-white cursor-pointer hover:scale-110 hover:shadow-lg': cell.day && cell.status === 'full' && !cell.isPast && !cell.isInRange,
                      'text-black cursor-pointer hover:scale-110 hover:shadow-lg': cell.day && cell.status === 'available' && !cell.isPast && !cell.isInRange,
                    }" 
                  ...
                  <div v-if="isTooltipOpen" ref="tooltipRef"
                    ...
                    <div class="px-5 pt-5 pb-4">
                      <div class="flex items-start justify-between mb-3">
                        ...
                        <button class="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-600 hover:bg-red-500 hover:text-white transition-all duration-300 font-bold" @click="closeTooltip">×</button>
                      </div>
                      <div class="h-1 w-full bg-gradient-to-r from-[#1E71B8] to-[#73BE5D] rounded-full mb-4"></div>
                      <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                          ...
                          <span class="text-gray-700">{{ tooltipData.status }}</span>
                        </div>
                        <div class="flex justify-between">
                          <span class="font-semibold text-[#1E71B8]">Price:</span>
                          <span class="text-gray-700">{{ tooltipData.price }}</span>
                        </div>
                        <div v-if="tourType === 'Joiners' && packageDuration > 1" class="pt-2 border-t border-gray-200">
                          <div class="flex justify-between">
                            <span class="font-semibold text-[#1E71B8]">Duration:</span>
                            <span class="text-gray-700">{{ packageDuration }} days</span>
                          </div>
                        </div>
                        <div v-if="tourType === 'Exclusive' && selectedDate && selectedEndDate" class="pt-2 border-t border-gray-200">
                          <div class="text-xs text-[#1E71B8] font-semibold mb-1">Selected Range:</div>
                          <div class="text-xs text-gray-600">{{ formatHuman(selectedDate) }} - {{ formatHuman(selectedEndDate) }}</div>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div v-if="currentStep === 2" class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div class="space-y-6">
            <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 shadow-sm">
              <div class="flex justify-between items-center mb-4">
                <span class="text-sm font-bold text-[#1E71B8] uppercase tracking-wide">Travellers</span>
                <span class="text-sm font-bold text-[#1E71B8] uppercase tracking-wide">Rate</span>
                <span class="text-sm font-bold text-[#1E71B8] uppercase tracking-wide">Quantity</span>
              </div>
              ...
              <div class="bg-gradient-to-r from-[#1E71B8] to-[#2980c9] rounded-2xl px-6 py-5 mb-6 shadow-lg">
                <div class="flex items-center justify-between">
                  ...
                  <div class="text-white font-semibold text-lg">₱ {{ selectedPackageData.pax_rate }}</div>
                  <div class="flex items-center gap-2">
                    <button @click="pax > 1 && pax--" class="w-9 h-9 rounded-full bg-white text-[#1E71B8] font-bold text-lg flex items-center justify-center hover:bg-gray-100 hover:scale-110 transition-all duration-300 shadow-md">-</button>
                    <input type="text" :value="pax" class="w-16 h-9 rounded-xl text-center border-2 border-white/20 bg-white/10 text-white font-bold backdrop-blur-sm" readonly>
                    <button @click="pax++" class="w-9 h-9 rounded-full bg-white text-[#1E71B8] font-bold text-lg flex items-center justify-center hover:bg-gray-100 hover:scale-110 transition-all duration-300 shadow-md">+</button>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-gradient-to-r from-[#1E71B8] to-[#2980c9] rounded-2xl px-6 py-5 shadow-lg">
                <div class="flex items-center justify-between">
                  ...
                  <div class="text-white font-semibold text-lg">₱ {{ selectedPackageData.kids_pax_rate }}</div>
                  <div class="flex items-center gap-2">
                    <button @click="kidsPax > 1 && kidsPax--" class="w-9 h-9 rounded-full bg-white text-[#1E71B8] font-bold text-lg flex items-center justify-center hover:bg-gray-100 hover:scale-110 transition-all duration-300 shadow-md">-</button>
                    <input type="text" :value="kidsPax" class="w-16 h-9 rounded-xl text-center border-2 border-white/20 bg-white/10 text-white font-bold backdrop-blur-sm" readonly>
                    <button @click="kidsPax++" class="w-9 h-9 rounded-full bg-white text-[#1E71B8] font-bold text-lg flex items-center justify-center hover:bg-gray-100 hover:scale-110 transition-all duration-300 shadow-md">+</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <label class="block font-semibold mb-3 text-[#1E71B8] text-sm uppercase tracking-wide">Discount ID Images</label>
              <div class="space-y-3">
                <div v-if="discountImages.length > 0" class="grid grid-cols-3 gap-3">
                  <div v-for="image in discountImages" :key="image.id" class="relative group">
                    <img :src="image.preview" :alt="image.name" class="w-full h-20 object-cover rounded-xl border-2 border-gray-200 group-hover:border-[#1E71B8] transition-all duration-300" />
                    <button @click="removeImage(image.id)" class="absolute -top-2 -right-2 w-7 h-7 bg-red-500 text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg hover:bg-red-600 hover:scale-110">×</button>
                  </div>
                </div>
                <button @click="triggerFileInput" :disabled="discountImages.length >= 3" class="w-full py-3 px-4 bg-gradient-to-r from-blue-50 to-indigo-50 text-[#1E71B8] text-sm font-semibold rounded-2xl border-2 border-dashed border-[#1E71B8] hover:border-[#73BE5D] hover:from-green-50 hover:to-emerald-50 hover:text-[#73BE5D] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:border-[#1E71B8] disabled:hover:from-blue-50 disabled:hover:to-indigo-50">
                  <div class="flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                    <span>{{ discountImages.length >= 3 ? 'Maximum 3 Images' : 'Add Image' }}</span>
                  </div>
                </button>
                <input id="discount-image-upload" type="file" multiple accept="image/*" @change="handleImageUpload" class="hidden" />
              </div>
          ...
          <div class="bg-gradient-to-br from-[#1E71B8] via-[#2980c9] to-[#1E71B8] rounded-3xl p-8 flex flex-col justify-between min-h-[500px] shadow-2xl relative overflow-hidden">
            ...
            <div class="absolute bottom-0 left-0 w-64 h-64 bg-[#73BE5D]/10 rounded-full blur-3xl"></div>
            <div class="relative z-10 space-y-6">
              <div>
                <div class="text-2xl font-bold text-white mb-1">{{ selectedPackageData.package_name }}</div>
              ...
              <div class="space-y-3 bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                ...
                <div class="flex items-center gap-2 text-white/90">
                  ...
                  <span class="font-semibold ml-auto">{{ durationDays }} Day<span v-if="durationDays !== 1">s</span></span>
                </div>
                <div class="flex items-center gap-2 text-white/90">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
              ...
              <div class="space-y-3">
                <div class="text-white/80 text-sm font-semibold uppercase tracking-wide mb-2">Travellers Breakdown</div>
                ...
                <div class="flex justify-between items-center text-white/90">
                  <span class="text-sm">Kids ₱{{ selectedPackageData.kids_pax_rate }} × {{ kidsPax }}</span>
            ...
            <button @click="submitBooking" class="relative z-10 w-full rounded-2xl py-4 font-bold text-lg transition-all duration-300 bg-[#73BE5D] text-white hover:bg-[#6aae56] hover:shadow-2xl hover:scale-105 active:scale-95 flex items-center justify-center gap-2 mt-6">
      ...
      <div class="px-8 pb-8 flex justify-between items-center border-t border-gray-100 pt-6">
        <button v-if="currentStep > 0" @click="prevStep" class="px-8 py-3 bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 rounded-2xl font-semibold hover:from-gray-200 hover:to-gray-300 transition-all duration-300 hover:shadow-lg hover:scale-105 active:scale-95 flex items-center gap-2">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-else></div>
        <button v-if="currentStep < 2" @click="nextStep" class="px-8 py-3 bg-gradient-to-r from-[#1E71B8] to-[#2980c9] text-white rounded-2xl font-semibold hover:from-[#2980c9] hover:to-[#1E71B8] transition-all duration-300 hover:shadow-lg hover:scale-105 active:scale-95 flex items-center gap-2">
          Next
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const showTooltip = (event, dateKey) => {
  ...
  tooltipData.value = {
    ...
    slotsTotal: info ? info.slotsTotal : selectedPackageData.value.capacity || 0,
...
const handleClickOutside = (event) => {
  ...
  const clickedInsidePackage = packageEl && packageEl.contains && packageEl.contains(target)
