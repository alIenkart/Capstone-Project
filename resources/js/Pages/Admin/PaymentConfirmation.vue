<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-8">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 mb-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div class="relative flex-1 max-w-md">
            ...
            <input v-model="searchQuery" type="text" placeholder="Search by ID, customer name, entry, method..."
            ...
            <div class="relative" data-filter-container>
              ...
              <div v-if="isFilterOpen" data-filter-menu
                ...
                <div v-for="option in filterOptions" :key="option" @click="handleFilterSelect(option)" :class="[
                  ...
                  statusFilter === option ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                ]">
                  <span :class="[
                    'font-medium',
                    statusFilter === option ? 'text-blue-700' : 'text-gray-700'
                  ]">
                    {{ option }}
                  </span>
                  <svg v-if="statusFilter === option" class="w-5 h-5 text-blue-600 ml-auto" fill="currentColor"
                    viewBox="0 0 20 20">
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gradient-to-r from-[#1E71B8] to-[#2a8bb5] text-white">
                <th class="px-6 py-4 text-center text-sm font-semibold">Payment ID</th>
                ...
                <th class="px-6 py-4 text-center text-sm font-semibold">Payment Entry</th>
                <th class="px-6 py-4 text-center text-sm font-semibold">Status</th>
                <th class="px-6 py-4 text-center text-sm font-semibold">Payment Method</th>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
          <div class="flex items-center justify-between">
            <p class="text-sm text-gray-700">
              Showing <span class="font-semibold">{{ filteredPayments.length > 0 ? 1 : 0 }}</span> to <span
                class="font-semibold">{{ filteredPayments.length }}</span> of <span class="font-semibold">{{
                filteredPayments.length }}</span> results
            </p>
    ...
    <ApprovalPaymentModal v-if="showApprovalPaymentModal" :payment="selectedPayment"
