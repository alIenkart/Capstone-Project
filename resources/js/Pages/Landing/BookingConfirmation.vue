  ...
  <div class="w-full min-h-screen bg-gradient-to-br from-[#f8fafc] to-[#f1f5f9] py-8">
    <div class="flex items-center gap-2 mb-12 mt-2 w-full max-w-6xl justify-center mx-auto px-4">
      ...
      <div class="flex items-center gap-1">
        ...
        <span class="ml-1 text-[#008DDA] font-semibold text-sm md:text-base">Reservation Details</span>
      </div>
      <span class="w-8 md:w-16 h-0.5 bg-gradient-to-r from-[#008DDA] to-[#1E71B8] mx-2"></span>
      <div class="flex items-center gap-1">
        ...
        <span class="ml-1 text-[#1E71B8] font-semibold text-sm md:text-base">Confirmation</span>
      </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row w-full max-w-7xl justify-center gap-8 mx-auto px-4">
      <div class="flex-1">
        <div class="flex justify-start mb-6">
          <button @click="emit('back')" class="px-6 py-2.5 rounded-lg font-semibold text-sm transition-all duration-300 bg-white text-[#1E71B8] border border-[#1E71B8] hover:bg-[#1E71B8] hover:text-white shadow-md hover:shadow-lg">
            ← Back
          </button>
        </div>

        <div class="mb-8">
          <h2 class="text-3xl font-bold text-[#1E71B8] mb-6 flex items-center gap-2">
          ...
          <form class="bg-white rounded-2xl border border-gray-200 shadow-md p-6 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <label class="block text-[#1E71B8] font-bold text-sm uppercase tracking-wide mb-2">First Name</label>
                <input type="text" v-model="first_name" :class="['w-full border-2 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#1E71B8] focus:border-transparent transition-all duration-300', errors.first_name ? 'border-red-500' : 'border-gray-200 hover:border-gray-300']" />
                <p v-if="errors.first_name" class="text-red-600 text-xs mt-2 font-medium">{{ errors.first_name }}</p>
              </div>

              <div>
                <label class="block text-[#1E71B8] font-bold text-sm uppercase tracking-wide mb-2">Last Name</label>
                <input type="text" v-model="last_name" :class="['w-full border-2 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#1E71B8] focus:border-transparent transition-all duration-300', errors.last_name ? 'border-red-500' : 'border-gray-200 hover:border-gray-300']" />
                <p v-if="errors.last_name" class="text-red-600 text-xs mt-2 font-medium">{{ errors.last_name }}</p>
              </div>

              <div>
                <label class="block text-[#1E71B8] font-bold text-sm uppercase tracking-wide mb-2">Email</label>
                <input type="email" v-model="email" :class="['w-full border-2 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#1E71B8] focus:border-transparent transition-all duration-300', errors.email ? 'border-red-500' : 'border-gray-200 hover:border-gray-300']" />
                <p v-if="errors.email" class="text-red-600 text-xs mt-2 font-medium">{{ errors.email }}</p>
              </div>

              <div>
                <label class="block text-[#1E71B8] font-bold text-sm uppercase tracking-wide mb-2">Contact No.</label>
                <input type="text" v-model="phone_number" :class="['w-full border-2 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#1E71B8] focus:border-transparent transition-all duration-300', errors.phone_number ? 'border-red-500' : 'border-gray-200 hover:border-gray-300']" />
                <p v-if="errors.phone_number" class="text-red-600 text-xs mt-2 font-medium">{{ errors.phone_number }}</p>
              </div>

              <div class="md:col-span-2">
                <label class="block text-[#1E71B8] font-bold text-sm uppercase tracking-wide mb-2">Address</label>
                <input type="text" v-model="address" :class="['w-full border-2 rounded-xl px-4 py-3 bg-gray-50 text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-[#1E71B8] focus:border-transparent transition-all duration-300', errors.address ? 'border-red-500' : 'border-gray-200 hover:border-gray-300']" />
                <p v-if="errors.address" class="text-red-600 text-xs mt-2 font-medium">{{ errors.address }}</p>
              </div>
            </div>

            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
              <div class="flex items-start gap-3">
                <input type="checkbox" id="agree" v-model="agreeChecked" class="accent-[#1E71B8] mt-1 w-5 h-5 cursor-pointer" />
                <label for="agree" class="text-gray-700 text-sm leading-relaxed">
                  I confirm that I have read and agree to the
                  <a href="#" @click.prevent="openModal('terms')" class="text-[#1E71B8] font-semibold underline hover:text-[#155E9C] transition-colors">Terms and Conditions</a>
                  and
                  <a href="#" @click.prevent="openModal('privacy')" class="text-[#1E71B8] font-semibold underline hover:text-[#155E9C] transition-colors">Privacy Policy</a>.
                </label>
              </div>
            </div>
            ...
            <button
              ...
              :class="['w-full py-3 text-white font-bold rounded-lg shadow-lg text-lg transition-all duration-300 flex items-center justify-center gap-2', (!agreeChecked || isLoading) ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-[#1E71B8] to-[#155E9C] hover:shadow-xl active:scale-95']"
            >
      ...
      <div class="w-full lg:w-96">
        <div class="bg-gradient-to-br from-[#1E71B8] to-[#155E9C] rounded-2xl p-6 shadow-lg text-white overflow-hidden relative h-fit sticky top-8">
          ...
          <div class="relative z-10">
            <div class="mb-5 pb-5 border-b border-white border-opacity-20">
              <p class="text-white text-opacity-70 text-xs uppercase tracking-widest font-semibold mb-1">Booking Summary</p>
              <h2 class="text-2xl font-bold">{{ booking.packageDestination }}</h2>
            </div>
            ...
            <div class="space-y-4 mb-6">
              ...
              <div class="flex items-center justify-between bg-white bg-opacity-10 rounded-lg p-3">
                ...
                <span class="font-semibold">{{ booking.duration }} Day<span v-if="durationDays !== 1">s</span></span>
              </div>
            ...
            <div class="space-y-3 mb-5 text-sm">
              <div class="flex justify-between">
                <span class="text-white text-opacity-80">Adults × {{ booking.adultsQuantity }}</span>
                <span class="font-semibold">₱{{ (booking.adultRate * booking.adultsQuantity).toLocaleString() }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-white text-opacity-80">Kids × {{ booking.kidsQuantity }}</span>
                <span class="font-semibold">₱{{ (booking.kidsRate * booking.kidsQuantity).toLocaleString() }}</span>
              </div>
              ...
              <div class="flex justify-between items-center text-lg">
                ...
                <span class="text-[#73BE5D] font-bold text-2xl">₱{{ (booking.amountWithDiscount).toLocaleString() }}</span>
              </div>
            </div>

            <div class="bg-[#73BE5D] bg-opacity-20 border border-[#73BE5D] rounded-lg p-3 text-xs text-center font-semibold">
